...
export function MobileNav({ initial }: { initial: string }) {
  ...
  return <>
    ...
    <nav className="bottom-nav" aria-label="Main navigation">
      <Link className={path === "/dashboard" ? "active" : ""} href="/dashboard" prefetch={true}><span>⌂</span>Home</Link>
      <Link className="nav-add" href="/groups/new" aria-label="Create group" prefetch={true}>+</Link>
      <Link className={path === "/groups" || path.startsWith("/groups/") ? "active" : ""} href="/groups" prefetch={true}><span>▦</span>Groups</Link>
    </nav>
  </>;
}
