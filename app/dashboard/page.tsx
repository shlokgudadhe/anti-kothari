...
export default async function Dashboard(){
  ...
  return <><Nav/><main className="terminal-shell">
    ...
    <section className="terminal-section"><div className="terminal-section-title"><h2>YOUR GROUPS</h2><Link className="brutal-mini" href="/groups/new" aria-label="Create group">+</Link></div>
      {groups.length?<div className="terminal-list">{groups.map((g:any,index:number)=>{const balance=g.owed-g.owing;return <Link className="terminal-group" href={"/groups/"+g.id} key={g.id} prefetch={true}><span className={"color-key "+rails[index%rails.length]}>{g.name.slice(0,1).toUpperCase()}</span><span className="terminal-group-copy"><b>{g.name}</b><small>Open group</small></span><b className={balance>0?"cash-in":balance<0?"cash-out":"cash-flat"}>{balance>0?"+":balance<0?"−":""}{balance===0?"settled":"₹"+Math.abs(balance).toFixed(2)}</b></Link>})}</div>:<div className="terminal-empty"><p>No groups yet.</p><Link className="brutal-button violet-fill" href="/groups/new" prefetch={true}>Create your first group</Link></div>}
    </section>
  </main></>;
}
