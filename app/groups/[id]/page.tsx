...
export default async function Group({params}:{params:Promise<{id:string}>}){
  ...
  return <><Nav/><main className="terminal-shell group-terminal">
    ...
    <div className="member-stack">{people.slice(0,5).map((p:any,index:number)=><span key={p.id} className={"member-disc "+colors[index%colors.length]}>{(p.full_name||p.email).slice(0,1).toUpperCase()}</span>)}<Link href={"/groups/"+id+"/members"} className="member-disc" style={{background:"#f2efe9",fontSize:"16px",textDecoration:"none"}}>+</Link></div>
